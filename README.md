...
跨平台应用自动稳定性门禁技能。它从源码、发布差异和历史缺陷生成可执行测试，验证测试自身能发现故障，在真实设备或机器上执行功能、升级兼容、资源和耐久门禁，并在授权范围内自动诊断、修复、重编译和回归，最终只对精确候选字节给出 `PASS` 或 `BLOCK`。

完整目录 `app-release-stability-gate/` 是独立安装单元。它适用于移动端、桌面端、Web/PWA、设备端及带服务端的应用；发布动作仍应使用对应平台或项目发布技能。
